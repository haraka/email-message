...
//
// Like lib/rfc2231.js this is a hand-written, forward-only character scanner
// rather than a regular expression. A regex form of this grammar has to search
// for the `?=` terminator with a lazy `.+?` / `[\s\S]*?` body; because the body
// can span arbitrarily far and overlaps at every `=?` start position, it
// backtracks quadratically on crafted header values — the same denial-of-service
// class as the RFC 2231 parser (GHSA-6f4j-gpc2-6p2g). A scan that never revisits
// a consumed character is O(n) for all inputs, and keeps that guarantee
// structural instead of resting on a fragile regex character class.
