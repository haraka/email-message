...
const max_mime_depth = config.get('mailparser.max_mime_depth') || 100

// RFC 2046 §5.1.1:
//   delimiter        = "--" boundary [transport-padding] CRLF
//   close-delimiter  = "--" boundary "--" [transport-padding] CRLF
// `--${boundary}` must be followed only by whitespace/CRLF, or by `--`
// (close) followed by whitespace/CRLF. A line like `--abcXYZ` is *not* a
// delimiter even though it shares the prefix `--abc`.
function matchMimeBoundary(line, boundary) {
  if (!boundary) return null
  const prefix = `--${boundary}`
  if (!line.startsWith(prefix)) return null
  const tail = line.slice(prefix.length)
  if (tail.startsWith('--')) {
    return /^[ \t]*\r?\n?$/.test(tail.slice(2)) ? 'close' : null
  }
  return /^[ \t]*\r?\n?$/.test(tail) ? 'open' : null
}
...
class Body extends EventEmitter {
  ...
  constructor(header, options, depth = 0) {
    super()
  ...
  parse_child(line) {
    ...
    const boundaryKind = matchMimeBoundary(line_string, this.#boundary)

    if (boundaryKind) {
      line = this.children[this.children.length - 1].parse_end(line)

      if (boundaryKind === 'close') {
        this.state = 'end'
      } else {
        this.emit('mime_boundary', line_string)
        const bod = new Body(new Header(), this.options, this.#depth + 1)
        for (const ln of [
          'attachment_start',
          'attachment_data',
          'attachment_end',
  ...
  parse_start(line) {
    ...
    if (/^(?:text|message)\//i.test(ct) && !/^attachment/i.test(cd)) {
      ...
      this.#boundary = match ? match[1].trim() : ''
      if (!this.#boundary) {
        logger.logwarn(`Multipart message without boundary: ${ct}`)
        this.state = 'body'
      } else {
  ...
  parse_multipart_preamble(line) {
    ...
    if (matchMimeBoundary(line_string, this.#boundary) === 'open') {
      this.emit('mime_boundary', line_string)
      const depthCap = this.options?.maxMimeDepth ?? max_mime_depth
      if (this.#depth >= depthCap) {
        logger.logerror(`Max MIME depth exceeded: ${this.#depth}`)
        this.state = 'end'
        return line
      }
      const bod = new Body(new Header(), this.options, this.#depth + 1)
  ...
  parse_attachment(line) {
    ...
    const boundaryKind = matchMimeBoundary(line_string, this.#boundary)
    if (boundaryKind) {
      if (boundaryKind === 'open') this.state = 'headers'
      return line
    }
