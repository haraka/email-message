...
function _parse_rfc2231(params, str) {
  // RFC 2231 extended parameters always contain both `*` and `=`. Without
  // both, no match is possible — and the greedy character-class below
  // catastrophically backtracks on long unstructured input (e.g. a 15KB
  // `From:` value with no `*=`), turning a no-op call into ~100ms+.
...
class Header {
  ...
  get_addresses(key) {
    const val = this.get_decoded(key)
    if (!val) return []
    return parseHeader(val)
  }
