...
describe('header', function () {
    ...
    describe('parse', function () {
        ...
        it('get_decoded', function (done) {
            ...
            assert.equal(this.h.get_decoded('fromUTF8'), 'Kohl’s <Kohls@s.kohls.com>');
            done()
        })
        it('content type w/parens', function (done) {
            assert.equal(this.h.lines().length, 13);
    ...
    describe('add_headers', function () {
        ...
        it('add_utf8', function (done) {
            ...
            assert.equal(this.h.lines()[0], 'Bar: =?UTF-8?Q?b=C3=B8=C3=B8?= 1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890\n');
            assert.equal(this.h.get_decoded('Bar'), 'bøø 1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890');
            done()
        })
    })
    ...
    describe('continuations', function () {
        beforeEach(_set_up)
        it('continuations_decoded', function (done) {
    ...
    describe('remove', function () {
        beforeEach(_set_up)
        it('removes only specified header', function (done) {
            ...
            assert.ok(this.h.header_list.find(name => name === 'X-Test-1: do-not-remove-me\n'));
            done()
        })
        it('removes multiple matching headers', function (done) {
            this.h.add('X-Test', 'remove me')
            ...
            assert.ok(this.h.header_list.find(name => name === 'X-Test-No: leave me\n'));
            done()
        })
    })

    describe('decode', function () {
        it('multiline 8bit header (#2675)', function (done) {
            this.h = new Header();
            this.h.parse ([
            ...
            assert.ok(this.h.get_decoded('content-disposition').includes('講演会案内書＆申込書改.txt'));
            done()
        })
        it('unfolding (#2702)', function (done) {
            this.h = new Header();
            this.h.parse ([
            ...
            assert.equal(this.h.get_decoded('subject'), 'Die beliebtesten CAD- und AVA-Programme; die klügsten Köpfe der Branche; Abschluss eines BIM-Pilotprojekts; Bauen in Zeiten des Klimawandels; u.v.m');
            done()
        })
    })
})
