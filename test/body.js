...
describe('body', function () {
    ...
    describe('basic', function () {
        ...
        it('correct mime parsing (#2548)', function (done) {
            ...
            tests.forEach(data => {
                ...
                assert.equal(data[3], body.children[0].bodytext, `charset: ${data[0]}, encoding: ${data[1]}`);
            });
    ...
    describe('banners', function () {
        ...
        it('insert_banner', function (done) {
            ...
            assert.equal(new_buf.toString(), "winter <P>htmlbanner</P></html>",
            ...
            new_buf = insert_banners_fn (content_type, enc, buf);
            assert.equal(new_buf.toString(), "winter\ntextbanner\n",
        ...
        it('insert_banner_empty_buffer', function (done) {
            ...
            assert.equal(new_buf.toString(), "<P>htmlbanner</P>",
                "empty html part gets a banner" );

            content_type = 'text/plain';
            new_buf = insert_banners_fn (content_type, enc, empty_buf);
            assert.equal(new_buf.toString(), "\ntextbanner\n",
    ...
    describe('rfc2231', function () {
        it('multi-value', function (done) {
            ...
            assert.ok(body.children[0].header.get_decoded('content-type').indexOf('URL="ftp://cs.utk.edu/pub/moore/bulk-mailer/bulk-mailer.tar";') > 0);
            assert.ok(body.children[1].header.get_decoded('content-disposition').indexOf('filename="marketron_lbasubmission_FTA Cricket Brentwood 3006445 Jackson TN_08282017.xlsx"') > 0);
            done()
        })
        ...
        it('enc-and-lang', function (done) {
            ...
            assert.ok(body.children[0].header.get_decoded('content-type').indexOf('title="This is ***fun***";') > 0);
            done()
        })
        ...
        it('multi-value-enc-and-lang', function (done) {
            ...
            assert.ok(body.children[1].header.get_decoded('content-type').indexOf('title="This is even more ***fun*** isn\'t it!";') > 0);
            done()
        })
    })
